...
def expand_coordinates(start: Tuple[int, int], end: Tuple[int, int]) -> List[Tuple[int, int]]:
    ...
    if start_x == end_x:
        ...
        range_x = range(start_x, end_x + 1) if start_x <= end_x else range(start_x, end_x - 1, -1)
        range_y = range(start_y, end_y + 1) if start_y <= end_y else range(start_y, end_y - 1 , -1)

        return list(zip([x for x in range_x], [y for y in range_y]))
