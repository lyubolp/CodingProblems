...
def bfs(grid: dict[Point, str], start: Point, start_direction: str, bounds: Point) -> set[tuple[Point, str]]:
    queue = [(start, start_direction)]
...
if __name__ == "__main__":
    ...
    possible_starts_left = [(Point(i, 0), 'R') for i in range(bounds.i)]
    possible_starts_right = [(Point(i, bounds.j - 1), 'L') for i in range(bounds.i)]
    possible_starts_top = [(Point(0, j), 'D') for j in range(bounds.j)]
    possible_starts_bot = [(Point(bounds.i - 1, j), 'U') for j in range(bounds.j)]

    possbile_starts = possible_starts_top + possible_starts_bot + possible_starts_left + possible_starts_right

    results = [len(bfs(grid, start_point, start_direction, bounds)) for start_point, start_direction in possbile_starts]
    result = max(results)
    print(result)

    # visited = bfs(grid, Point(0, 3), 'D', bounds)
