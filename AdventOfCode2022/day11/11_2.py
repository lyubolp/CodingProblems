...
# The important thing is to which monkey the items should go to - a.k.a what the test returns.
# The numbers that are going to be used for the test are the first 8 prime numbers
# Another thing to know, is that: N % M = k <=> (N % M) % M = k

# So, if we apply modular divison with the MAGIC_CONSTANT (product of the first 8 prime numbers),
#   we will make the numbers a lot more managable
...
class Monkey:
    ...
    def single_turn(self, item) -> Tuple[int, int]:
        ...
        worry_level = worry_level % MAGIC_CONSTANT
